...
useEffect(() => {
  const fetchCompetitions = async (parentId: number) => {
    try {
      ...
      console.log('Fetching competitions for parentId:', parentId);
      const res = await axios.get(`http://192.168.227.138:8080/api/competitions/competitions/parent/${parentId}`, {
  ...
  const getInformationsByParent = async (parentId: number) => {
    try {
      ...
      console.log('Fetching informations for parentId:', parentId);
      const response = await axios.get(`http://192.168.227.138:8080/api/informations/by-parent/${parentId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
useEffect(() => {
  const loadParentData = async () => {
    try {
      ...
      const response = await axios.get('http://192.168.227.138:8080/api/parents/me', {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
